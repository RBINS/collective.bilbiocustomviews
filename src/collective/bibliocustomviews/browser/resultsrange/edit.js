FacetedEdit.ResultsRangeWidget = function(wid, context){
  this.wid = wid;
...
FacetedEdit.ResultsRangeWidget.prototype = {
...
FacetedEdit.initializeResultsRangeWidget = function(){
  jQuery('div.faceted-resultsrange-widget').each(function(){
      var wid = jQuery(this).attr('id');
      wid = wid.split('_')[0];
      FacetedEdit.Widgets[wid] = new FacetedEdit.ResultsRangeWidget(wid);
  });
};
...
jQuery(document).ready(function(){
  jQuery(FacetedEdit.Events).bind(
    ...
    FacetedEdit.initializeResultsRangeWidget);
});
